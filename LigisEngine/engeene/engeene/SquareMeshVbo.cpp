...
void SquareMeshVbo::loadData()
{
  ...
  glBufferData(GL_ARRAY_BUFFER, 3 * 4 * sizeof(GLfloat), texCoorData, GL_STATIC_DRAW);
...
void SquareMeshVbo::render()
{
  ...
  if (GLRendererEditor::getInstance()->getTexLo() != -1) {
	  glBindBuffer(GL_ARRAY_BUFFER, this->texVboId);
	  glVertexAttribPointer(GLRendererEditor::getInstance()->getTexLo(), 2, GL_FLOAT, GL_FALSE, 2 * sizeof(GLfloat), NULL);
  }
